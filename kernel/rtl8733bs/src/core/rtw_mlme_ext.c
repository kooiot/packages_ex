...
_continue:
	...
	printk("%s: check_fwstate111 pmlmepriv->fw_state: %x - %x\n", __func__, pmlmepriv->fw_state, WIFI_STATION_STATE);
	if (check_fwstate(pmlmepriv, WIFI_STATION_STATE))
		return _SUCCESS;

	printk("%s: check_fwstate222 pmlmepriv->fw_state: %x - %x\n", __func__, pmlmepriv->fw_state, WIFI_ASOC_STATE);
	...
	printk("%s: check_fwstate 3333 pmlmepriv->fw_state: %x - %x\n",pmlmepriv->fw_state, WIFI_ASOC_STATE);
	if (check_fwstate(pmlmepriv, WIFI_ASOC_STATE) == _TRUE &&
		...
		if (psta == NULL) {
			...
			if (rtw_is_list_empty(&psta->asoc_list)) {
				...
				printk("%s: asoc_list insert tail\n", __func__);
